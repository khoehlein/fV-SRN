...
class EnsembleConverter(object):
    ...
    def convert_variable(self, var_name, drop_nan_levels=False, attach_velocity=True, clamp_min=None, clamp_max=None, max_member=None):
    ...
    def _convert_variable_in_file(self, file_name, var_name, drop_nan_levels, attach_velocity, clamp_min, clamp_max):
        ...
        for i, t in enumerate(variable.coords[Axis.TIME.value]):
            ...
            self._store_snapshot(file_name, var_name, i, variable_data, velocity_data)
    ...
    def _store_snapshot(self, file_name, var_name, i, variable_data, velocity_data):
        full_data = variable_data
        if velocity_data is not None:
            full_data = np.concatenate([full_data, velocity_data], axis=0)
        storage_path = self._get_storage_path(file_name, var_name, i)
    ...
    def _get_storage_path(self, file_name, var_name, i):
        member_id = file_name.split('.')[0]
        return os.path.join(self.target_dir, var_name, '{}_t{:02d}.cvol'.format(member_id, i))
...
def build_parser():
    ...
    parser.set_defaults(overwrite=False, create=False, drop_nan_levels=False, normalize=False,attach_velocity=False)
