...
from volnet.experiments.multigrid_preprocessing.initial_clustering import InitialClustering
...
def build_parser():
    ...
    parser.add_argument('--apply-primary-initialization', dest='primary_initialization', action='store_true')
    parser.set_defaults(primary_initialization=False)
    return parser
...
def apply_initialization(args: dict, network: PyrendererSRN, dataset: WorldSpaceDensityData):
    if args['network:latent_features:ensemble:mode'] == 'multi-grid' and args['primary_initialization']:
        clustering = InitialClustering(seed=42)
        clustering.initialize_mixing_features(dataset, network.latent_features.ensemble_features)
    if args['network:latent_features:volume:mode'] == 'multi-grid' and args['primary_initialization']:
        clustering = InitialClustering(seed=42)
        clustering.initialize_mixing_features(dataset, network.latent_features.volumetric_features)
...
def main():
    ...
    apply_initialization(args, network, training_data)
