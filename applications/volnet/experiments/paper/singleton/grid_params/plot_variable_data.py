...
def draw_new_plot():
    ...
    pn = 12 * 352 * 250
    variables = ['tk', 'rh']
    fig, ax = plt.subplots(2, len(variables), figsize=(6, 4),gridspec_kw={'hspace': 0.1})

    for i, variable_name in enumerate(variables):
        ax[0, i].set_yscale('log')
        ax[1, i].set_yscale('log')
        ax[0, i].set(title=f'Variable {variable_name}', xticks=[], xticklabels=[])
        ax[1, i].set(xlabel='compression ratio', xticks=[], xticklabels=[])
        data = pd.read_csv(
            os.path.join(directory, variable_name, 'num_channels', 'accuracies.csv'))
        ...
        for normalization in ['level-min-max_scaling', 'local-min-max_scaling', 'global-min-max_scaling']:
            ...
            norm_data_mean = norm_data.groupby('num_parameters').mean()
            norm_data_std = norm_data.groupby('num_parameters').std()
            ax[0, i].errorbar(np.log(pn / norm_data_mean.index), norm_data_mean['l2'],
                              yerr=norm_data_std['l2'], label=normalization.split('-')[0])
            ax[1, i].errorbar(np.log(pn / norm_data_mean.index), norm_data_mean['l2r'],
                              yerr=norm_data_std['l2r'], label=normalization.split('-')[0])

        labels = [10, 20, 50]
        ax[1, i].set(xticks=np.log(labels), xticklabels=labels)

    ax[0, 0].set(ylim=(.001,.1))
    ax[0, 1].set(ylim=(.005,.15))
    labels = [0.05, 0.1]
    ax[1, 0].set(ylim=(.05,0.15), yticks=labels, yticklabels=labels, )
    ax[1,0].yaxis.set_minor_locator(NullLocator())
    labels = [1., 2., 5.]
    ax[1, 1].set(ylim=(1., 5.), yticks=labels, yticklabels=labels, )
    ax[1, 1].yaxis.set_minor_locator(NullLocator())
    ax[0, 0].set(ylabel='RMSE (rescalved)')
    ax[1, 0].set(ylabel='RMSE (original)')

    plt.tight_layout()
    plt.savefig('accuracy_vs_normalization.pdf')
    plt.show()
