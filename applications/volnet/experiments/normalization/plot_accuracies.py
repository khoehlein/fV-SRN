...
def plot_new_figure():
    ...
    variable_names = ['tk', 'rh']
    pn = 12 * 352 * 250
    fig, ax = plt.subplots(2, len(variable_names), figsize=(8, 4), sharex='all', gridspec_kw={'hspace': 0.})
    ...
    for i, variable_name in enumerate(variable_names):
        ...
        for normalization in ['level', 'local', 'global']:
            norm_data = data.loc[data['normalization'] == normalization, :]
            norm_data = norm_data.groupby('num_parameters')
            norm_data = norm_data.mean()
            compression_ratio = pn / norm_data.index.values
            order = np.argsort(compression_ratio)
            ax[0, i].plot(compression_ratio, norm_data['rmse_rescaled'], label=f'{normalization} scaling', marker='.')
            ax[1, i].plot(compression_ratio, norm_data['rmse_reverted'], label=f'{normalization} scaling', marker='.')
        ax[0, i].legend()
        ax[1, i].legend()
