                ...
                if epoch in epochs_with_save:
                    with torch.no_grad():
                        ...
                        for j, data_tuple in enumerate(training_data.visualization_dataloader()):
                            ...
                            image.paste(convert_image(prediction), box=(left+posX*img_size, posY*single_height))
                            image.paste(convert_image(target), box=(left + posX * img_size, posY*single_height+img_size))
                            ...
                            y = centered_text("TF=%d, Time=%.2f, Ensemble=%.2f"%(tf_index, time_index, ensemble_index),
                        ...
                        imageio.imwrite(os.path.join(logdir, "e%d.png" % epoch), img_np)
