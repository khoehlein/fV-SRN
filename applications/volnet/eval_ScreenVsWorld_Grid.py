...
def get_args_and_hdf5_file(training_mode, config_file_index:int, stepsize:float,
    ...
    parametersWorld = [
        ...
        "--logdir", 'volnet/results/eval_ScreenVsWorldGrid/log',
        "--modeldir", 'volnet/results/eval_ScreenVsWorldGrid/model',
        "--hdf5dir", 'volnet/results/eval_ScreenVsWorldGrid/hdf5',
        '--name', output_name,
        '--save_frequency', '50'
    ]
    parametersScreen = [
    ...
    hdf5_file = 'volnet/results/eval_ScreenVsWorldGrid/hdf5/' + output_name + ".hdf5"
...
def eval(configs):
    print("Evaluate")
    statistics_file = 'volnet/results/eval_ScreenVsWorldGrid/stats.json'
    ...
    #rendering_mode_screen = LoadedModel.EvaluationMode.TENSORCORES_MIXED
    rendering_mode_screen = LoadedModel.EvaluationMode.PYTORCH16
    rendering_mode_world = LoadedModel.EvaluationMode.TENSORCORES_MIXED
    ...
    def load_and_save(training_mode, cfg_index, stepsize, direction_mode):
        ...
        ln = LoadedModel(filename, force_config_file=CONFIG_FILES[cfg_index])
    ...
    for cfg_index in range(len(CONFIG_FILES)):
        image_folder = 'volnet/results/eval_ScreenVsWorldGrid/images_%d/'%cfg_index
        local_stats = {
            'cfg_index': cfg_index,
        ...
        for s in STEPSIZES:
            ...
            if reference_images is None:
                ...
                for i in range(num_cameras):
                    reference_images[i] = ln.render_reference(cameras[i], width, height, timestep=VOLUME_TIMESTEP)
                    imageio.imwrite(
                        os.path.join(image_folder_reference, 'reference%03d.png' % i),
                        LoadedModel.convert_image(reference_images[i]))

            max_steps = max([ln_screen[0][0].get_max_steps(c, width, height, s) for c in cameras])
            local_stats['steps_%d' % (1000 * s)] = max_steps
            print(f"Stepsize {s} -> max steps {max_steps}")
            ...
            for ln, name in ln_screen:
                ...
                time, ssim, lpips = compute_stats(
                    ln, rendering_mode_screen, reference_images, s,
            ...
            time, ssim, lpips = compute_stats(
                ln, rendering_mode_world, reference_images, s,
            ...
            image_folder_density = os.path.join(image_folder, "world_pre_%d" % (1000 * s))
            os.makedirs(image_folder_density, exist_ok=True)
            ...
            time, ssim, lpips = compute_stats(
                ...
                os.path.join(image_folder_density, 'world%03d.png'),
                True, True)
            local_stats['world_pre_%d' % (1000 * s)] = {
                'time': time,
                'ssim': ssim,
                'lpips': lpips,
            }
...
def make_plots(statistics_file):
    ...
    classNames = ["pos", "dirP", "dirS", "dirF", "world", "world-pre"]
    numClasses = len(classNames)
    def classTags(tf_idx):
        return [f"run_screen_{tf_idx}_%d_plain", f"run_screen_{tf_idx}_%d_dirD", f"run_screen_{tf_idx}_%d_dirS",
            f"run_screen_{tf_idx}_%d_dirF", "world_%d", "world_pre_%d"]
    ...
    for row in range(numTFs):
        ...
        tags = classTags(row)
        sorted_ssim = dict()
        sorted_lpips = dict() #defaultdict(lambda: 1.0)
        for j, s in enumerate(STEPSIZES):
        ...
        # images
        shutil.copyfile(
            os.path.join(output_folder, "images_%d/reference/reference000.png" % (row)),
            os.path.join(output_folder, "%d_reference.png" % row))
        for tag in classTags(row):
            for s in STEPSIZES:
                foldername = tag % (1000*s)
                filename = "world000.png" if foldername.startswith("world") else "screen000.png"
                shutil.copyfile(
                    os.path.join(output_folder, "images_%d/%s/%s" % (row, foldername, filename)),
                    os.path.join(output_folder, "%s_%s.png" % (row, foldername)))

    lgd = fig.legend(
        handles, handle_names,
        bbox_to_anchor=(0.65, 0.7), loc='lower center', borderaxespad=0.)
    fig.savefig(os.path.join(output_folder, 'ScreenVsWorldGrid-SSIM.%s'%FILETYPE),
