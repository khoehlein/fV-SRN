...
def eval(configs):
    print("Evaluate")
    statistics_file = os.path.join(BASE_PATH, 'stats-time2.json')
    ...
    for cfg_index, config in enumerate(configX):
        ...
        for network_idx, (name, ln) in enumerate(lns.items()):
            if ln is None:
                print("\n===================================== (% 2d/% 2d) Skip network (network is None)" % (
                network_idx + 1, len(network_combinations) + len(grid_only_combinations)), name)
                continue
        ...
        for idx, volumetricFeatures in enumerate(grid_only_combinations):
            ...
            if os.path.exists(os.path.join(image_folder_network, 'stats.json')):
                print("\n===================================== (% 2d/% 2d) Skip network (already done)" % (
                    len(network_combinations) + idx + 1, len(network_combinations) + len(grid_only_combinations)),
                      name)
                with open(os.path.join(image_folder_network, 'stats.json'), 'r') as f:
                    network_stats = json.load(f)
            else:
                ...
                def getKeyframe(t: int, return_volume):
                    ...
                    print(
                        f"Data: shape={data.shape}, min={data[-1, ...].min().item()}, max={data[-1, ...].max().item()}")
                ...
                def render_interpolated(ensemble: int, timestep: int, cam: int,
                                        timer: pyrenderer.GPUTimer) -> torch.Tensor:
...
def make_plots(statistics_file):
    ...
    for configIdx, config in enumerate(configX):
        ...
        fig.savefig(os.path.join(output_folder, 'TimeVolumetricFeatures-%s.%s'%(config[0], FILETYPE)),
