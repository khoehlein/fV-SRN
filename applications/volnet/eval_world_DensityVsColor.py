...
def get_args_and_hdf5_file(output_mode, config_file_index:int):
    ...
    parameters = [
        ...
        "--logdir", 'volnet/results/eval_world_DensityVsColor/log',
        "--modeldir", 'volnet/results/eval_world_DensityVsColor/model',
        "--hdf5dir", 'volnet/results/eval_world_DensityVsColor/hdf5',
        '--name', output_name,
        '--save_frequency', '50'
    ]
    hdf5_file = 'volnet/results/eval_world_DensityVsColor/hdf5/' + output_name + ".hdf5"
...
def eval(configs):
    print("Evaluate")
    statistics_file = 'volnet/results/eval_world_DensityVsColor/stats.json'
    ...
    for cfg_idx, cfg in enumerate(CONFIG_FILES):
        image_folder = 'volnet/results/eval_world_DensityVsColor/images_%d/'%(cfg_idx)
...
def make_plots(statistics_file):
    ...
    fig.savefig(os.path.join(output_folder, 'DensityVsColor-SSIM.%s'%FILETYPE),
