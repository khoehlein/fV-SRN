...
def get_args_and_hdf5_file(output_mode, config_file_index:int):
    ...
    parameters = [
        ...
        "--logdir", 'volnet/results/eval_world_DensityVsColorGrid/log',
        "--modeldir", 'volnet/results/eval_world_DensityVsColorGrid/model',
        "--hdf5dir", 'volnet/results/eval_world_DensityVsColorGrid/hdf5',
        '--name', output_name,
        '--save_frequency', '50'
    ]
...
def eval(configs):
    print("Evaluate")
    statistics_file = 'volnet/results/eval_world_DensityVsColorGrid/stats.json'
    ...
    for cfg_idx, cfg in enumerate(CONFIG_FILES):
        image_folder = 'volnet/results/eval_world_DensityVsColorGrid/images_%d/'%(cfg_idx)
...
def make_plots(statistics_file):
    ...
    fig.savefig(os.path.join(output_folder, 'DensityVsColorGrid-SSIM.%s'%FILETYPE),
