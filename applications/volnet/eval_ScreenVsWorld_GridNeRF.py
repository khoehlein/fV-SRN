"""
Script to evaluate the network quality if it is trained in world-space or screen-space.
For fourier features, the construction by NeRF is used.
"""
...
GRID_RESOLUTION = 32
GRID_CHANNELS = 16
BASE_PATH = 'volnet/results/eval_ScreenVsWorldGridNeRF'
...
def get_args_and_hdf5_file(training_mode, config_file_index:int, stepsize:float,
    ...
    parametersWorld = [
        ...
        "--lr_step", "100",
        "-i", "200",
        '--fouriercount', str((BEST_NETWORK[0]-4)//2), '--fourierstd', '-1',
    ...
    parametersScreen = [
        ...
        "-lr", "0.01",
        "-i", "200",
        '--fouriercount', str(num_fourier_screen), '--fourierstd', '-1',
        "--activation", BEST_ACTIVATION,
        "--layers", ':'.join([str(BEST_NETWORK[0])]*(BEST_NETWORK[1]-1)),
        "--volumetric_features_resolution", str(GRID_RESOLUTION),
        "--volumetric_features_channels", str(GRID_CHANNELS),
...
def eval(configs):
    ...
    if os.name != 'nt':
        rendering_mode_screen = rendering_mode_world = LoadedModel.EvaluationMode.PYTORCH16
    else:
    ...
    for cfg_index in range(len(CONFIG_FILES)):
        image_folder = BASE_PATH+'/images_%d/'%cfg_index
        local_stats = {
            'cfg_index': cfg_index,
...
def make_plots(statistics_file):
    ...
    def classTags(tf_idx):
        return [f"run_screen_{tf_idx}_%d_plain", f"run_screen_{tf_idx}_%d_dirD",
