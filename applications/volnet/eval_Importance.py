...
BASE_PATH = 'volnet/results/eval_Importance'
FILENAME_PATTERN = "importance-world-%s-%s-%s-%s"
...
def collect_configurations():
    cfgs = []
    for config, network, fourier, importance in itertools.product(configX, networkX, fourierX, importanceX):
        filename = FILENAME_PATTERN % (
            config[0], network[0], fourier[0], importance[0])
        cfgs.append((config[1], network[1:], fourier[1], importance[2], filename))
    return cfgs
...
def get_args_and_hdf5_file(cfg):
    ...
    common_parameters = [
        ...
        "--lr_step", "50",
        "-i", "200",
        "--logdir", 'volnet/results/eval_Importance/log',
        "--modeldir", 'volnet/results/eval_Importance/model',
        "--hdf5dir", 'volnet/results/eval_Importance/hdf5',
    ]
    ...
    config, network, fourier, importance, filename = cfg
...
def make_plots(statistics_file):
    ...
    for row in range(numRows):
        ...
        for col, (name, tag) in enumerate(zip(statNames, statTags)):
            ...
            for ni, (network_label, network_channels, network_layers) in enumerate(networkX):
                ...
                for i,(impN, impH, impV) in enumerate(importanceX[:-numExtraImportance]):
                    filename = FILENAME_PATTERN % (
                        configX[row][0], network_label, fourier, impN)
        ...
        density_filenames = [FILENAME_PATTERN % (configX[row][0], network_label, fourier, importanceX[-2][0])
                            for network_label, network_channels, network_layers in networkX]
        density_filename = min(density_filenames, key=lambda filename: local_stat[filename][tag][0])
        color_filenames = [
            FILENAME_PATTERN % (configX[row][0], network_label, fourier, importanceX[-1][0])
            for network_label, network_channels, network_layers in networkX]
    ...
    fig.savefig(os.path.join(output_folder, 'Importance-SSIM.%s'%FILETYPE),
